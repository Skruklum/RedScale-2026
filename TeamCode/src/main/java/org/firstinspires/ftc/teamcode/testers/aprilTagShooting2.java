...
public class aprilTagShooting2 extends OpMode {
    ...
    private DcMotorEx intakeMotor; // <<< INTAKE ADDED
    ...
    public void init() {
        ...
        intakeMotor = hardwareMap.get(DcMotorEx.class, "intake"); // <<< INTAKE ADDED
    ...
    public void loop() {
        ...
        double turretDeg = turretMotor.getCurrentPosition() / TICKS_PER_DEGREE;
        double absoluteDegreeRelativeToRobot =
                normalizeAngle(robotYaw + turretDeg);
        ...
        if (isAutoAim) {
            ...
            for (AprilTagDetection d : detections) {
                ...
                if (d.id == activeGoalTagId) {
                    if (d.ftcPose.range != 0) AprilTagDistance = d.ftcPose.range;
                    if (d.ftcPose.bearing != 0) AprilTagDataA = d.ftcPose.bearing;
                }
            ...
            double speedMod = Range.clip(
                    0.6 + (-gamepad2.right_stick_y * 0.4), 0.2, 1.0);
        ...
        if (tagVisible) {
            ShooterSolution shooterSolution =
                    shooterController.getBestShootingSolution(
                            inchToCm(AprilTagDistance), 98.5);

            shooterAd.setPosition(
                    shooterController.angleToServo(shooterSolution.bestAngle));

            shooterMotor.setVelocity(
                    shooterController.rpmToVelocityTicks(
                            shooterSolution.targetRPM));
        ...
        telemetry.addData("MODE",
                isAutoAim ? (tagVisible ? "AUTO (VISION)" : "AUTO (GYRO)") : "MANUAL");
        telemetry.addData("PID",
                isAutoAim ? (usingVisionGains ? "Vision" : "Gyro") : "Manual");
        telemetry.update();
    }
    ...
    private void initVision() {
        Position cameraPosition =
                new Position(DistanceUnit.CM,
                        cmToInch(-21.5), mmToInch(39.11), cmToInch(41), 0);
    ...
    public static class CameraStreamProcessor
            implements VisionProcessor, CameraStreamSource {

        private final AtomicReference<Bitmap> lastFrame =
                new AtomicReference<>(Bitmap.createBitmap(
                        1, 1, Bitmap.Config.RGB_565));

        public void init(int width, int height,
                         CameraCalibration calibration) {
            lastFrame.set(Bitmap.createBitmap(
                    width, height, Bitmap.Config.RGB_565));
        }

        public Object processFrame(Mat frame, long captureTimeNanos) {
            Bitmap b = lastFrame.get();
            if (b.getWidth() != frame.width()
                    || b.getHeight() != frame.height()) {
    ...
    public double cmToInch(double cm) { return cm / 2.54; }
